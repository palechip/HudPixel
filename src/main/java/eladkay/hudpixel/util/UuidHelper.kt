...
class UuidHelper(uuid: String, internal var callback: UuidCallback) : Thread() {
    ...
    private fun getUsernameFormUUID(uuid: String) {
        ...
        // Did it fail?
        if (name == null) {
            LoggerHelperMG.logWarn("Failed to the username for the UUID. Using fallback API...")
            try {
                ...
                LoggerHelperMG.logWarn("Failed to use the fallback API for $uuid @ $FALLBACK_API$uuid")
                name = "!ERROR!"
            }
